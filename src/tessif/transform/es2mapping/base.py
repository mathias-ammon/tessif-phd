"""
# interfaces/transform/es2mapping/base.py
...
from tessif.visualize.dcgrph import greyscale2hex
...
class ESTransformer(abc.ABC):
    ...
    optimized_es : energy_system
        Object returned by a supported energy system simulation library. See
    ...
    def node_data(self):
        ...
        return {attr_name: getattr(self, attr_name)
                for attr_name in node_attribute_names}
    ...
    def edge_data(self):
        ...
        for attr in dir(self):
            if (any(include in attr for include in including) and
                    not any(exclude in attr for exclude in excluding)):

                _edge_data.append(attr)

        return {attr_name: getattr(self, attr_name)
                for attr_name in _edge_data}
    ...
    def dump(self, directory=None, filename=None):
        """
        Store (dump) the es transformer into ``directory.filename``.

        Parameters
        ----------
        directory : str, default=None
            Path the created es transformer is dumped to.
        ...
        filename : str, default=None
            :func:`~pickle.dump` the es transformer using this name.
        ...
        if not directory:
            d = os.path.join(paths.write_dir, 'tsf')
        else:
            d = directory
        ...
        if not filename:
            f = 'es_transformer.tsf'
        else:
            f = filename

        pickle.dump(self, open(os.path.join(d, f), 'wb'))

        msg = 'Stored Tessif energy system transformer in {}'.format(
            os.path.join(d, f))
...
class ScaleResultier(Resultier):
    ...
    def __init__(self, optimized_es, **kwargs):
        ...
        self._number_of_constraints = self._map_number_of_constraints(
            optimized_es)

    @property
    def number_of_constraints(self):
        """
        Number of constraints the software used to describe the given model.
        """
    ...
    def _map_inflows(self):
        ...
        for node, load in self.node_load.items():
            ...
            for col_name, series in df.iteritems():
                if (df.columns.to_list().count(col_name) > 1 and
                        (series <= 0).all()):
                    pass
                else:
...
class CapacityResultier(Resultier):
    ...
    def __init__(self, optimized_es, **kwargs):
        ...
        self._installed_capacities = self._map_installed_capacities(
            optimized_es)
        self._original_capacities = self._map_original_capacities(
            optimized_es)
        ...
        for group, uid_attribute in self.groupings.items():
            ...
            for representation, uid in self.uid_nodes.items():
                ...
                if c == defaults.energy_system_nodes[uid_attribute]:
                    group_dict[defaults.energy_system_nodes[
                        'unspecified']].append(representation)
                else:
...
class FlowResultier(Resultier):
    ...
    def __init__(self, optimized_es, **kwargs):
        ...
        self._specific_emissions = self._map_specific_emissions(
            optimized_es)
    ...
    def _map_net_energy_flows(self, optimized_es):
        ...
        for node in self.nodes:
            for inflow in self.node_inflows[node].columns:
                _net_energy_flows[
                    nts.Edge(inflow, node)] = round(self.node_inflows[
                        node][inflow].sum(axis='index'), 2)
    ...
    def _map_edge_weights(self):
        ...
        if max_costs > 0:
            for key in self._specific_flow_costs.keys():
                ...
                _edge_weights[key] = (
                    self.edge_specific_flow_costs[key] /
                    max_costs
                )

                if _edge_weights[key] < defaults.nxgrph_visualize_defaults[
                        'edge_minimum_weight']:
...
class LabelFormatier(Resultier):
        ...
        for node in self.nodes:
            ...
            if not isinstance(inst_cap, collections.abc.Iterable):
                ...
                if inst_cap and characteristic_value:
                    ...
                    if getattr(self.uid_nodes[
                            node], 'component') in spellings.storage:
    ...
    def _map_edge_labels(self):
        ...
        for edge in self.edges:
            ...
            if not any([self.uid_nodes[node].component in spellings.storage
                        for node in edge]):
...
class MplLegendFormatier(Resultier):
        ...
        for component, color in themes.colors.component.items():
            if color in self._nformats.node_color.component.values():
                node_marker = mpl.lines.Line2D(
                    ...
                    markeredgecolor=color, markersize=15, linestyle='')
        ...
        for node in sorted(self.nodes):
            ...
            if self._nformats.node_size[node] == 'variable':
                ...
                most_outer_circle = mpl.lines.Line2D(
                    [], [], marker=marker, markerfacecolor=color, alpha=0.3,
                    markeredgecolor=color, linestyle='', markersize=15)

                outer_circle = mpl.lines.Line2D(
                    [], [], marker=marker, markerfacecolor=color, alpha=0.6,
                    markeredgecolor=color, markersize=10, linestyle='')

                inner_circle = mpl.lines.Line2D(
                    [], [], marker=marker, markerfacecolor=color,
                    markeredgecolor=color, markersize=5, linestyle='')
                ...
                base_size = self.node_characteristic_value[
                    node]
    ...
    def _create_node_style_legend(self):
        ...
        most_outer_circle = mpl.lines.Line2D(
            [], [], marker='o', markerfacecolor='black', alpha=0.3,
            markeredgecolor='black', markersize=15, linestyle='')

        outer_circle = mpl.lines.Line2D(
            [], [], marker='o', markerfacecolor='black', alpha=0.6,
...
class NodeFormatier(Resultier):
        ...
        if drawutil not in ['dc', 'nx']:
            logger.warning(
                'Tried to access nonexistent field {} from {}.'.format(
                    drawutil, __name__))
            logger.warning(
                "Use one of the existing fields: {}".format(
                    "['dc', 'nx']"))
    ...
    def _map_nx_node_shapes(self):
        ...
        component_types = [
            'bus', 'connector', 'sink', 'storage', 'transformer']
        ...
        for node in self.nodes:
            ...
            if self.uid_nodes[node].component.lower() in component_types:
                for component_type in component_types:
                    ...
                    if self.uid_nodes[node].component in getattr(
                        ...
                        _nx_node_shape[
                            node] = defaults.nxgrph_node_shapes.get(
                                component_type)
            ...
            # Source
                ...
                if any(expr in node
                       for expr in esci.name['photovoltaic']):
                    _nx_node_shape[
                        node] = self._default_node_shapes.get('solar')

                # ... unless its a wind onshore source
                if any(expr in node
                       for expr in esci.name['onshore']):
                    _nx_node_shape[
                        node] = self._default_node_shapes.get('wind')

                # ... unless its a wind offshore source
                if any(expr in node
                       for expr in esci.name['offshore']):
                    _nx_node_shape[
                        node] = self._default_node_shapes.get('wind')
                ...
                if any(expr in node
                       for source in [
                        'gas', 'oil', 'lignite', 'hardcoal', 'nuclear']
            ...
            if self.uid_nodes[node].component.lower() in component_types:
                for component_type in component_types:
                    ...
                    if self.uid_nodes[node].component in getattr(
                            spellings, component_type):
                        _dc_node_shape[
                            node] = defaults.dcgrph_node_shapes.get(
                            component_type)
        ...
        for node in self.nodes:
            ...
            if inst_cap == defaults.energy_system_nodes['variable_capacity']:
                ...
                _node_size[node] = round(
                    inst_cap / self.node_reference_capacity *
    ...
    def _map_node_colors(self):
        ...
        for node in self.nodes:
            ...
            if not _component_grouped_node_colors[node]:
                _component_grouped_node_colors[
                    node] = defaults.nxgrph_visualize_defaults[
                        'node_color']
            ...
            for key, variations in esci.component.items():
                if hasattr(self.uid_nodes[node], 'component') and any(
                    ...
                    _component_grouped_node_color_maps[
                        node] = next(themes.ccycles.component[key])
                        ...
                        if any(tag in node for tag in variations):
                            _name_grouped_node_color_maps[node] = next(
                                getattr(themes.ccycles, category).get(
                                    key,
                                    cycle(defaults.nxgrph_visualize_defaults[
                                        'node_color_map'])))
...
class EdgeFormatier(Resultier):
    ...
    def __init__(self, optimized_es, drawutil='nx', cls=None, **kwargs):
        ...
        if self._drawutil == 'dc':
            ...
            _cls = ([0, .33, .66], ['dotted', 'dashed', 'solid'])
    ...
    def _map_nx_edge_width(self):
        ...
        for edge in self.edges:
            _edge_width[edge] = round(
                ...
                defaults.nxgrph_visualize_defaults['edge_width'], 2)

            if _edge_width[edge] < defaults.nxgrph_visualize_defaults[
                    'edge_minimum_width']:
    ...
    def _map_dc_edge_width(self):
        ...
        for edge in self.edges:
            _edge_width_cyt[edge] = round(
                self.edge_net_energy_flow[edge] /
                self.edge_reference_net_energy_flow *
                defaults.dcgrph_visualize_defaults['edge_width'], 2)
    ...
    def _map_nx_edge_colors(self):
        ...
        for edge in self.edges:
            edge_color = round(self.edge_specific_emissions[
                edge] / self.edge_reference_emissions, 2)

            if edge_color < defaults.nxgrph_visualize_defaults[
                    'edge_minimum_grey']:
                edge_color = defaults.nxgrph_visualize_defaults[
                    'edge_minimum_grey']
    ...
    def _map_dc_edge_colors(self):
        ...
        for edge in self.edges:
            ...
            _edge_colors[edge] = greyscale2hex(edge_color_float)
    ...
    def _map_edge_linestyles(self):
        ...
        for edge in self.edges:
            # scale color to emission/max_emissions
            edge_cost_float = self.edge_specific_flow_costs[edge] / \
                max_costs
...
class ICRHybridier(Resultier):
    ...
    def __init__(self, optimized_es,
                 node_formatier, edge_formatier,
