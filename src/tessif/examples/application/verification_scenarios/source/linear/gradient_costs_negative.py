...
mapping = {
    'sources': {
        'sourceA': {
            ...
            'flow_gradients': {
                'electricity':
                    nts.PositiveNegative(positive=float('+inf'), negative=1)
            },
            'gradient_costs': {
                'electricity':
                    nts.PositiveNegative(positive=0, negative=2)
            },
            'timeseries': {
                'electricity':
...
}  # minimum working example source - linear - gradient_costs_negative
...
mapping_advanced = {
    'sources': {
        'source_1': {
            ...
            'flow_costs': {'electricity': [0, 0, 0, 0, 0, 10, 10, 10, 10, 10]},
            # flow_costs force a change in terms fo the active source
            'gradient_costs': {
        ...
        'source_2': {
            ...
            'outputs': ('electricity',),
            'flow_rates': {'electricity': nts.MinMax(min=0, max=10)},  # flow_rate between 0 and 10 (same as sink)
            'flow_costs': {'electricity': [10, 10, 10, 10, 10, 0, 0, 0, 0, 0]},
            # flow_costs force a change in terms fo the active source
            'gradient_costs': {
